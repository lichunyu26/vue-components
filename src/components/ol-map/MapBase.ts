...
// @ts-ignore
// @ts-ignore
import {OSM, WMTS as WMTSSource, XYZ} from "ol/source";
...
// @ts-ignore
import {fromLonLat, get as getProjection, getTransform, transform} from 'ol/proj';
...
export function initMap(options: InitMapOptions) {
    ...
    //用于记录上次触发hover的覆盖物
    let _hoverFeature: any = undefined;
    _map.on("pointermove", event => {
        ...
        if (feature === _hoverFeature) return;
        console.log(feature)
        if (feature) {
 ...
 * 处理地图色系
 * @param pixelsTemp
 * 参考 https://blog.csdn.net/linzi19900517/article/details/120307650
 */

/*
function _u_reverseFunc(pixelsTemp: any) {
    //蓝色
...
// @ts-ignore
window._u_reverseFunc = _u_reverseFunc;
*/
...
export function createLayer(options: CreateLayerOpts, blue = false) {
    ...
    // @ts-ignore
    let projection: any = getProjection(projs[options.proj]);
    let projectionExtent = projection.getExtent();
    let origin = projectionExtent ? getTopLeft(projectionExtent) : [-180, 90];
    let fromLonLat = getTransform('EPSG:4326', projection as any);
 ...
 */
export function createLayerWithSlife(url: string) {
 ...
 */
function createFeature(coordinates: number[],
                       index?: number,
                       _coordinates?: number[] | Array<number[]>,
                       featureFn?: Function | string,
                       iconFn?: Function) {
 ...
 */
export function createPoint(coordinates: number[] | Array<number[]>, featureFn?: Function | string, iconFn?: Function) {
    const vectorSource = new VectorSource({
        features: [],
    });
    if (Array.isArray(coordinates[0])) {
        // @ts-ignore
        vectorSource.addFeatures(coordinates.map((item: number[], index) => {
            const _feature = createFeature(item, index, coordinates, featureFn, iconFn)
            return _feature
        }))
    } else {
        //单点
        const _feature = createFeature(coordinates as number[], 0, coordinates, featureFn, iconFn)
 ...
 * @return vectorLayer 返回当前生成的layer
 */
export function createLine(coordinates: number[][], featureFn?: Function) {
...
function TrackPlaybackBase(coordinates: number[][],
                           initIndex = 0) {
...
//轨迹回放
export function TrackPlayback(coordinates: number[][],
                              initIndex = 0,
                              cb: Function,
                              map?: any) {
    if (!cb) {
    ...
    const {
        ...
        route,
        styles
    } = TrackPlaybackBase(coordinates, initIndex);
 ...
 * @constructor
 */
export function TrackPlaybackSinglePop(coordinates: number[][],
                                       initIndex = 0,
                                       cb: Function,
                                       map?: any) {
    //如果起始位置不为0，生成之前的线
 ...
 */
const _overlayMap: any = {}

/**
 *  判断图层是否存在，只判断该对象是否存在于_overlayMap中，至于是否叠加在地图上请使用 overlayInMap 获取
 * @param name 图层名称，唯一值
 */
export function overlayIsRegister(name: string): boolean {
 ...
 * @param name 图层名称，唯一值
 */
export function overlayInMap(name: string): boolean {
 ...
 * @param visible 是否显示
 */
export function visibleLayer(name: string, visible: boolean) {
    ...
    Object.keys(_overlayMap).forEach(key => {
        ...
        if (key === name ||
            ...
            _overlayMap[key].multiple) return;
        //移除覆盖物
 ...
 */
export function bingOverlay2Map(name: string, overlay: import("ol/layer/Base.js").default | undefined | null, multiple: boolean = false,) {
    //如果不存在，初始化该对象
    if (!_overlayMap[name]) {
        _overlayMap[name] = {
            multiple
    ...
    if (overlay) {
        ...
        if (_overlayMap[name].target) map.removeLayer(_overlayMap[name].target)
        //重新给覆盖物赋值
