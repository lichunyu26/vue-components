...
function findAbsoluteEl(el: HTMLElement): any {
    const position = window.getComputedStyle(el).position;
    if (position === "absolute") return el;
    // @ts-ignore
    return findAbsoluteEl(el.parentElement)
}
const v3App = createApp(App)
function findElParentWithClass(el: HTMLElement,className:string){
    const position = window.getComputedStyle(el).position;
    if (position === "absolute") return el;
...
const draggable = {
    mounted: function (bindEl: any) {
        ...
        let disx = 0;
        let disy = 0
        bindEl.onmousedown = function (e: any) {
